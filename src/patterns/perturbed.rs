    ...
    pub fn noise(point: &Tuple) -> f64 {
        ...
        Perturbed::lerp(
            ...
            Perturbed::lerp(
                ...
                Perturbed::lerp(
                    u,
                    Perturbed::grad(p[AA as usize], x, y, z),
                    Perturbed::grad(p[BA as usize], x - 1.0, y, z),
                ),
                Perturbed::lerp(
                    u,
                    Perturbed::grad(p[AB as usize], x, y - 1.0, z),
                    Perturbed::grad(p[BB as usize], x - 1.0, y - 1.0, z),
                ),
            ),
            Perturbed::lerp(
                v,
                Perturbed::lerp(
                    u,
                    Perturbed::grad(p[(AA + 1) as usize], x, y, z - 1.0),
                    Perturbed::grad(p[(BA + 1) as usize], x - 1.0, y, z - 1.0),
                ),
                Perturbed::lerp(
                    u,
                    Perturbed::grad(p[(AB + 1) as usize], x, y - 1.0, z - 1.0),
                    Perturbed::grad(p[(BB + 1) as usize], x - 1.0, y - 1.0, z - 1.0),
                ),
            ),
        )
    }

    fn permutation() -> [i64; 512] {
        [
            ...
            24, 72, 243, 141, 128, 195, 78, 66, 215, 61, 156, 180, 151, 160, 137, 91, 90, 15, 131,
            13, 201, 95, 96, 53, 194, 233, 7, 225, 140, 36, 103, 30, 69, 142, 8, 99, 37, 240, 21,
            10, 23, 190, 6, 148, 247, 120, 234, 75, 0, 26, 197, 62, 94, 252, 219, 203, 117, 35, 11,
